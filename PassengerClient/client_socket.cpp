...
string ClientSocket::GetEROR(bool& EROR){
    string ret;
    ret=this->receive();
    if(ret.find("EROR :")==string::npos)
        EROR=false;
    else{
        EROR=true;
        int pos=ret.find("EROR :");
        ret.erase(ret.begin()+pos,ret.begin()+pos+6);
    }
    return ret;
}
vector<string> ClientSocket::standard_input(string input_array)
{
    vector<string> ret;
    int i=0;
